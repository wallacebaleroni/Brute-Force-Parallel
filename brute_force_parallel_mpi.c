...
double varia(char **senha, char lim_inf[], char lim_sup[]) {
	...
	// Loop da variação
	MPI_Irecv(&achou, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &req);
...
int main() {
	...
	strcpy(objetivo, "01234567890123456789");
	...
	printf("Processo: %d	Limite Inferior: %s 	Limite Superior: %s\n", id_processo, str_lim_inf, str_lim_sup);
	...
	if(iteracao != -1) {
		...
		for(i = 0; i < n_processos; i++) {
			if(i != id_processo) {
				MPI_Request req;
				MPI_Isend(&achou, 1, MPI_INT, i, 0, MPI_COMM_WORLD, &req);
			}
		}
	}
